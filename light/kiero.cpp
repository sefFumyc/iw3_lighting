...
kiero::Status::Enum kiero::init(RenderType::Enum _renderType)
{
	...
	if (_renderType != RenderType::None)
	{
		if (_renderType >= RenderType::D3D9 && _renderType <= RenderType::D3D12)
		{
			...
			// ============================================================
			// D3D9 初始化逻辑 (已修改：增加 Debug 输出和 NULL_REF 修复)
			// ============================================================
			if (_renderType == RenderType::D3D9)
			{
#if KIERO_INCLUDE_D3D9
				std::cout << "[Kiero Debug] Loading d3d9.dll..." << std::endl;
				HMODULE libD3D9;
				if ((libD3D9 = ::GetModuleHandle(KIERO_TEXT("d3d9.dll"))) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					std::cout << "[Kiero Error] d3d9.dll not found!" << std::endl;
				...
				if ((Direct3DCreate9 = ::GetProcAddress(libD3D9, "Direct3DCreate9")) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					std::cout << "[Kiero Error] Direct3DCreate9 export not found!" << std::endl;
					return Status::UnknownError;
				}

				std::cout << "[Kiero Debug] Creating Direct3D9 Interface..." << std::endl;
				LPDIRECT3D9 direct3D9;
				if ((direct3D9 = ((LPDIRECT3D9(__stdcall*)(uint32_t))(Direct3DCreate9))(D3D_SDK_VERSION)) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					std::cout << "[Kiero Error] Failed to create Direct3D9 Interface!" << std::endl;
					return Status::UnknownError;
				}
				...
				if (direct3D9->GetAdapterDisplayMode(D3DADAPTER_DEFAULT, &displayMode) < 0)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					std::cout << "[Kiero Error] GetAdapterDisplayMode failed!" << std::endl;
					return Status::UnknownError;
				}
				...
				std::cout << "[Kiero Debug] Attempting to create Dummy Device..." << std::endl;
				...
				{
					std::cout << "[Kiero Warn] HAL Device creation failed! Trying NULL_REF..." << std::endl;

					// 尝试 2: 使用 NULL_REF (这是一个更稳妥的 Dummy Device 模式，只获取函数表，不真的渲染)
					if (direct3D9->CreateDevice(D3DADAPTER_DEFAULT, (D3DDEVTYPE)4, window, D3DCREATE_SOFTWARE_VERTEXPROCESSING | D3DCREATE_DISABLE_DRIVER_MANAGEMENT, &params, &device) < 0)
					{
						direct3D9->Release();
						::DestroyWindow(window);
						::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
						std::cout << "[Kiero Error] All CreateDevice attempts failed!" << std::endl;
						return Status::UnknownError;
					}
				}

				std::cout << "[Kiero Debug] Device Created! Copying VTable..." << std::endl;
